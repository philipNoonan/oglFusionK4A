...
namespace rgbd
{
	...
	)
	{
		...
		progs["Integrate"]->setUniform("maxWeight", maxWeight);
		progs["Integrate"]->setUniform("K", K);
		progs["Integrate"]->setUniform("invK", invK);

		progs["Raycast"]->setUniform("largeStep", largeStep);
		progs["Raycast"]->setUniform("step", step);
		progs["Raycast"]->setUniform("nearPlane", nearPlane);
		progs["Raycast"]->setUniform("farPlane", farPlane);
	...
	{
		glm::mat4 view = T * glm::inverse(kMat);
		progs["Raycast"]->use();
		...
		dstFrame.getVirtualVertexMap(0)->bindImage(1, 0, GL_WRITE_ONLY);
		dstFrame.getVirtualNormalMap(0)->bindImage(2, 0, GL_WRITE_ONLY);

		glDispatchCompute(dstFrame.getWidth() / 32, dstFrame.getHeight() / 32, 1);
